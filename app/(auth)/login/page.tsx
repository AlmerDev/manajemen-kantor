...
export default async function LoginPage({ searchParams }: { searchParams: { error?: string; success?: string; email?: string } }) {
      ...
      <div className="guest-shell">
        <section className="guest-brand-col">
          <div className="guest-brand-col-inner">
            <div className="guest-brand-mark">{logoUrl ? (
              // eslint-disable-next-line @next/next/no-img-element
              <img src={logoUrl} alt="Logo kantor" width="54" height="54" />
            ) : <i className="bi bi-grid-1x2-fill" />}</div>
            <h2 className="guest-brand-title">{settings.office_name}</h2>
            <p className="guest-brand-desc">{settings.tagline || 'Masuk untuk mengelola data karyawan, absensi, cuti, payroll, kasbon, dan pengumuman kantor.'}</p>
            <ul className="guest-features">
              <li><i className="bi bi-people" /> Data karyawan dan jabatan</li>
              <li><i className="bi bi-calendar-check" /> Absensi harian dan bulanan</li>
              <li><i className="bi bi-wallet2" /> Payroll, kasbon, dan laporan</li>
            </ul>
          </div>
          <div className="guest-brand-footnote">{settings.footer_note || settings.panel_name || 'Office OS'}{settings.city ? ` • ${settings.city}` : ''}</div>
        </section>
        <section className="guest-form-col">
          <div className="guest-card">
            ...
            <form action={loginAction} autoComplete="on" data-loading-text="Memeriksa akun...">
              ...
              <div className="guest-check"><input type="checkbox" name="remember" id="remember" value="1" /><label htmlFor="remember" style={{ margin: 0, cursor: 'pointer', fontWeight: 500 }}>Ingat saya di perangkat ini</label></div>
              <button type="submit" className="guest-submit">Masuk</button>
            </form>
            <div className="guest-demo"><strong>Akun demo</strong><br />Email: admin@kantor.com<br />Password: password</div>
          </div>
        </section>
      </div>
    </div>
